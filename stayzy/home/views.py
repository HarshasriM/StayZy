...
from accounts.models import Hotel , HotelBooking, HotelUser,HotelVendor
...
def hotel_details(request, slug):
    ...
    if request.method == "POST":
        ...
        if not request.user.is_authenticated:
            messages.warning(request, "Please login to book the hotel.")
            return HttpResponseRedirect(request.path_info)
        
        booking_user =  HotelUser.objects.get(id = request.user.id)
        if(not booking_user):
            messages.warning(request,"Please login as Hotel User")
